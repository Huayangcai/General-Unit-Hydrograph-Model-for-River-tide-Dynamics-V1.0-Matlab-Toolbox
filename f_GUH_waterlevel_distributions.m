function [X,yy,yy_gradient,Xdata2,yy2,yy2_gradient,X1,X2,sigma]=f_GUH_waterlevel_distributions(inputs)

%% Water level distribution
xdata=inputs(:,1); % distance
ydata=inputs(:,2); % water level
...
yy=g(X,xdata); % computed water level
yy_gradient=u(X,xdata); % computed water level gradient

% spatial distributions of water level along the channel
